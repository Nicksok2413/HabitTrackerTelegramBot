"""
Обработчики общих команд бота (/start, /help, /cancel) и неизвестных сообщений.

Эти обработчики доступны из любого состояния (команды) или когда состояние не установлено (неизвестные сообщения).
"""
...
async def cmd_start(message: Message, api_client: HabitTrackerClient, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            ...
            f"Для справки по командам нажмите /help.\n\n"
...
async def cmd_help(message: Message) -> None:
    ...
    Выводит список доступных команд и краткое описание функционала.
    ...
    help_text = (
        ...
        "/cancel - Отменить текущее действие (создание, редактирование)\n\n"
        "<b>Как пользоваться:</b>\n"
        "1. Нажмите <b>«Создать привычку»</b>, чтобы добавить новую цель.\n"
        "2. В разделе <b>«Мои привычки»</b> можно отмечать выполнение, смотреть статистику и редактировать привычки.\n"
        "3. Бот будет присылать напоминания в указанное вами время.\n\n"
        "<i>Если что-то пошло не так, просто введите /start.</i>"
    )
...
async def handle_unknown_message(message: Message) -> None:
    ...
    Ловит любой текст, который не является командой и не соответствует
    ни одной кнопке меню, при условии, что пользователь не находится в процессе диалога (FSM).

    Args:
        message (Message): Объект сообщения Telegram.
    """
    # Если это не текст (например, стикер), игнорируем
    if not message.text:
        return

    await message.answer(
        "🤔 <b>Я не понимаю эту команду.</b>\n\n"
        "Пожалуйста, используйте кнопки меню или введите /help для справки.\n"
        "Если вы хотите создать привычку, нажмите кнопку <b>«Создать привычку»</b>.",
        reply_markup=get_main_menu_keyboard(),
    )
